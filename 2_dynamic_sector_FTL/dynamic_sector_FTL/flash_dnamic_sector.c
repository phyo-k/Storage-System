...
void init(const int inclination, int* const megabyte)
{
	int i = 0;
	flash data;
	FILE* fp = fopen("dynamic_sector_flash_memory.txt", "wb");
...
{
	FILE* fp = fopen("dynamic_sector_flash_memory.txt", "rb");
...
void ftl_write(FILE* fp, const int lsn, const char* string, int* const megabyte)
{
	...
	for (indicator = 0; (mapping+indicator)->psn <= limit * SECTOR; indicator++)
	{
		count++;
